...
class DeltoROSDriver(Node):
    ...
    def __init__(self):
        ...
        self.joint_state_pub = self.create_publisher(JointState,'gripper/joint_states',qos_profile)
        self.grasp_sub = self.create_subscription(Int32,'gripper/cmd',callback=self.grasp_callback, qos_profile=qos_profile)
        
        self.grasp_mode_sub = self.create_subscription(Float32MultiArray,'gripper/target_joint',callback=self.targejoint_callback, qos_profile=qos_profile)
        self.grasp_mode_sub2 = self.create_subscription(Float32Array,'gripper/target_joint',callback=self.targejoint_callback, qos_profile=qos_profile)
        self.joint_state_timer=self.create_timer(1/self.publish_rate, self.timer_callback)
    ...
    async def execute_callback(self, goal_handle):
        ...
        if self.joint_state_list:
            ...
            new_array = [[self._rad2deg(joints) for joints in subset] for subset in new_array]
    ...
    def targejoint_callback(self, msg):
    ...
    def waypointMove(self, waypointList, threshold):
        self.stop_thread = False
        self.waypoint_thread = threading.Thread(target=self._waypointMove, args=(waypointList, threshold))
